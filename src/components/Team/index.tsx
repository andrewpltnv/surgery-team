...
export default function Team() {
  return (
    <section id="team" className="w-full ~py-12/24">
      <div className="container mx-auto px-4 md:px-6" itemType="http://schema.org/Organization" itemScope>
        ...
        <div className="flex flex-col items-center justify-evenly gap-8 md:flex-row">
          {experts.map((expert) => {
            ...
            return <Member image={image} name={name} position={position} slug={slug} key={slug?.current} />
          })}
        </div>
      </div>
    </section>
...
export function Member({
  ...
  position,
  image,
}: Required<Pick<Expert, "name" | "position" | "image" | "slug">>) {
  return (
    <div
    ...
    >
      <Link aria-label={`${name} profile`} href={`/experts/${slug.current}`} className="absolute inset-0" />
      <Image
      ...
      />
      <h3 className="mx-4 mt-4 text-2xl font-semibold leading-8 tracking-wide" itemProp="name">
        {name}
      </h3>
      <Badge variant={"outline"} className="m-4 h-14 px-4 py-2" itemProp="jobTitle">
